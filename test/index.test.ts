...
describe('ng-git-gen', () => {
  ...
  beforeEach('create ng project', done => removeNgProject(e => e == null ?
    exec('ng new --routing --interactive=false  --skip-install --skip-tests --skip-git -st ' +
      `--directory '${path.join('test', project_name)}' '${project_name}'`, done) : done(e))
  );
  ...
  const confirmBaseModuleAndRoute = (route_name: string, cb: (error?: Error) => void) => {
    setTimeout(() => {
      ...
      if (!existsSync(route_dir)) return cb(new assert.AssertionError({ message: `'${route_dir}' not found` }));
      else if (!existsSync(gen_dir)) return cb(new assert.AssertionError({ message: `'${gen_dir}' not found` }));

      for (const fname of ['generated.module.ts', 'generated.routes.ts']) {
        const full_path = path.join(gen_dir, fname);
        if (!existsSync(full_path))
          return cb(new assert.AssertionError({ message: `'${fname}' not found in '${gen_dir} ['${full_path}']` }));
      }
      return cb();
    }, 1000);
  }
  '/Users/samuel/repos/ng-git-gen/test/ng-test/src/app/wiki/generated';
  '/Users/samuel/repos/ng-git-gen/test/ng-test/src/app/wiki/generated/generated.module.ts';

  const list_component = (route_name: string) => path.join(project_dir, 'src', 'app', route_name, 'list.component.ts');
  ...
  it(test0, done => {
    cmd
      .run(base_cli_args)
      .then(() => {
        confirmBaseModuleAndRoute('wiki', e => {
          if (e != null) return done(e);
          const wiki_list_component = list_component('wiki');
          if (existsSync(wiki_list_component))
            return done(new assert.AssertionError({ message: `'${wiki_list_component}' should not exist` }));
        });
      });
  });
